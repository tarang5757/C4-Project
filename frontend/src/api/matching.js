...
async function sendMatchNotification(matchId) {
  ...
  try {
    ...
    // Use the backend API URL from the environment variable
    // Access the environment variable injected by the build process
    const backendApiUrl =
      typeof process !== "undefined" && process.env.REACT_APP_BACKEND_API_URL
        ? process.env.REACT_APP_BACKEND_API_URL
        : "http://localhost:3000"; // Default to localhost if not set
    const notificationEndpoint = `${backendApiUrl}/api/send-match-notification`; // Corrected endpoint path to include /api
